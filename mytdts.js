...
var fs = require("fs");
var dir = "./data";
let nodeData = require("./data/mytdts");
...
async function update() {
  ...
  for (let i = 0; i < Object.keys(nodeData).length; i++) {
    ...
    let keepAddress = await deposit.methods.keepAddress().call();
    ...
    if (nodeData[i]["keepAddress"] != keepAddress) {
        nodeData[i]["keepAddress"] = keepAddress;
      }
  ...
  fs.writeFileSync("data/mytdts.json", data);
}
update();
